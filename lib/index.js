        ...
        if (ref.opts.library === node.source.value
          ...
          let newImports = specifiers.map(specifier => {
            if (isType.includes(`${specifier.local.name}`)) {
              return types.importDeclaration([types.importSpecifier(specifier.local)], types.stringLiteral(`${node.source.value}/lib/isType`)) // import { isFunction } from 'diana/lib/isType'
            }
            if (math.includes(`${specifier.local.name}`)) {
              return types.importDeclaration([types.importSpecifier(specifier.local)], types.stringLiteral(`${node.source.value}/lib/math`)) // import { isFunction } from 'diana/lib/isType'
            }
